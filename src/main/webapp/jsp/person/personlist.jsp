...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      <td><a class="badge badge-light" href="${baseurl}person/query/${person.id}">${person.firstName} - ${person.lastName}</a></td>
      <td>${persona.edad}</td>
      <td onmouseover="getAnioMatricula(persona)">${person.matricula}</td>
      <td>${person.modeloCoche}</td>
      <td><a class="badge badge-primary" href="editar-persona/${persona.id}"><fmt:message key="botones.modificar" /></a></td>
